...
<xsl:template name="format-month">
  ...
  <xsl:param name="month-word">
    <xsl:choose>
      <xsl:when test="$month = 01">Janeiro</xsl:when>
      <xsl:when test="$month = 02">Fevereiro</xsl:when>
      <xsl:when test="$month = 03">Março</xsl:when>
      <xsl:when test="$month = 04">Abril</xsl:when>
      <xsl:when test="$month = 05">Maio</xsl:when>
      <xsl:when test="$month = 06">Junho</xsl:when>
      <xsl:when test="$month = 07">Julho</xsl:when>
      <xsl:when test="$month = 08">Agosto</xsl:when>
      <xsl:when test="$month = 09">Setembro</xsl:when>
      <xsl:when test="$month = 10">Outubro</xsl:when>
      <xsl:when test="$month = 11">Novembro</xsl:when>
      <xsl:when test="$month = 12">Dezembro</xsl:when>
    </xsl:choose>
  </xsl:param>
  <xsl:choose>
