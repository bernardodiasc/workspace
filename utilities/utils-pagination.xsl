...
<xsl:template name="pagination">
  ...
  <xsl:param name="show-range" select="11" />
  ...
  <xsl:param name="class-selected" select="'active'" />
  <xsl:param name="class-ellipsis" select="'disabled'" />
...
<xsl:template name="pagination-numbers">
  ...
  <xsl:if test="$page = $page-first and $page-first &gt; 1">
    ...
    <xsl:if test="$page != 2">
      <li class="{$class-ellipsis}"><span>&#8230;</span></li>
    </xsl:if> 
  </xsl:if>
